...
// ---------------------------------------------------------------------
// Group info / settings — WhatsApp-style: tap the group header to see
// who's in the group, add members, or leave.
// ---------------------------------------------------------------------
...
function openGroupInfo() {
  ...
  document.getElementById('group-info-count').textContent = `${currentGroup.members.length} üzv`;
...
function renderGroupMembersList() {
  ...
  list.innerHTML = '';
  document.getElementById('group-members-label').textContent = `Üzvlər (${currentGroup.members.length})`;
...
document.getElementById('group-add-member-btn').addEventListener('click', async () => {
  ...
  if (!Number.isInteger(userId) || userId <= 0) {
    errEl.textContent = 'Düzgün istifadəçi ID-si daxil et (məs. 000002).';
  ...
  if (status !== 200) {
    errEl.textContent = 'Əlavə edilmədi. İstifadəçi ID-sini yoxla.';
    errEl.classList.remove('hidden');
    return;
  }
  ...
  if (refreshed.status === 200) {
    ...
    renderGroupMembersList();
    document.getElementById('group-info-count').textContent = `${currentGroup.members.length} üzv`;
    document.getElementById('group-chat-members').textContent = `${currentGroup.members.length} üzv`;
  }
});

document.getElementById('group-leave-btn').addEventListener('click', async () => {
  if (!currentGroup) return;
  if (!confirm(`"${currentGroup.name}" qrupundan çıxmaq istəyirsən?`)) return;
